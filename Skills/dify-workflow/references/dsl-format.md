...
This document describes the YAML structure of a Dify workflow DSL file, extracted from the Dify source code. The DSL version to use is configured in `config.yml`. This document covers the latest version (0.6.0); for older versions, see `version-deltas.md`.
